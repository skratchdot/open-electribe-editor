...
MergeService.merge(differences, true);
	...
	@Override
	public void write(EsxRandomAccess out, int patternNumber) throws EsxException, IOException {
		// Stop immediately if we are passed an invalid patternNumber
		if (patternNumber >= EsxUtil.NUM_PATTERNS || patternNumber < 0)
			throw new EsxException("Invalid patternNumber: " + patternNumber);

		// Jump to the start of patternNumber's data
		out.seek(EsxUtil.ADDR_PATTERN_DATA + (patternNumber * EsxUtil.CHUNKSIZE_PATTERN));
		...
		// byte 12
		out.writeByte(this.getFxChain().getValue());
		// byte 13
		out.writeByte(this.getLastStep().getValue());
		// byte 14
		...
		// byte 15
		out.writeByte(this.getArpeggiatorCenterNote().getValue());
		// byte 16~17
		out.writeShort(this.getMuteStatus());
		// bytes 18~19
		out.writeShort(this.getSwingStatus());
		// bytes 20-21
		out.writeShort(this.getOutputBusStatus());
		// bytes 22~23
		out.writeShort(this.getAccentStatus());

		// bytes 24~329 (34 bytes each)
		for (int i = 0; i < EsxUtil.NUM_PARTS_DRUM; i++) {
			this.getDrumParts().get(i).write(out, patternNumber, i);
		}

		// bytes 330~877 (274 bytes each)
		for (int i = 0; i < EsxUtil.NUM_PARTS_KEYBOARD; i++) {
			this.getKeyboardParts().get(i).write(out, patternNumber, i);
		}

		// bytes 878~973 (32 bytes each)
		for (int i = 0; i < EsxUtil.NUM_PARTS_STRETCHSLICE; i++) {
			this.getStretchSliceParts().get(i).write(out, patternNumber, i);
		}

		// bytes 974~1129 (156 bytes)
		for (int i = 0; i < EsxUtil.NUM_PARTS_AUDIOIN; i++) {
			this.getAudioInPart().write(out, patternNumber, i);
		}

		// bytes 1130~1147 (18 bytes)
		for (int i = 0; i < EsxUtil.NUM_PARTS_ACCENT; i++) {
			this.getAccentPart().write(out, patternNumber, i);
		}

		// bytes 1148~1159 (4 bytes each)
		for (int i = 0; i < EsxUtil.NUM_PARAMETERS_FX; i++) {
			this.getFxParameters().get(i).write(out, patternNumber, i);
		}

		// bytes 1160~4279 (130 bytes each)
		for (int i = 0; i < EsxUtil.NUM_PARAMETERS_MOTION; i++) {
			this.getMotionParameters().get(i).write(out, patternNumber, i);
		}
