...
public class EsxUtil {
	...
	public static final int SIZE_FILE_MAX = SIZE_FILE_MIN
			+ MAX_SAMPLE_MEM_IN_BYTES;
	 ...
	 */
	public static int findFirstIndex(EList<? extends EObject> collection,
			int startingIndex, ArrayList<Integer> skipIndices,
			EStructuralFeature feature, Object value, boolean allowLoop) {
		int returnIndex = -1;

		for (int i = 0; i < collection.size(); i++) {
			if (collection.get(i).eGet(feature).equals(value)
	 ...
	 * @return
	 */
	public static byte[] getByteArrayWithLength(String input, int length,
			byte fillByte) {
	 ...
	 * @return
	 */
	public static int packInt(int packedInt, int inputValue, int numBits,
			int numShiftedLeft) {
	...
	public static int getExistingSampleIndex(EsxFile esxFile, Sample sample) {
		for (int i = 0; i < EsxUtil.NUM_SAMPLES; i++) {
			...
			if (sample.getNumberOfSampleFrames() == currentSample
					.getNumberOfSampleFrames()
					&& Arrays.equals(sample.getAudioDataChannelBoth(),
							currentSample.getAudioDataChannelBoth())) {
				return i;
			}
		}
		return -1;
