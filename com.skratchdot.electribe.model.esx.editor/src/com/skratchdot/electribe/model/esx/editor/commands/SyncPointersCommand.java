...
public class SyncPointersCommand extends CompoundCommand {
	...
	private void syncAllPatternPointers() {
		...
		for(int i=0; i<EsxUtil.NUM_PATTERNS; i++) {
			...
			if(currentPattern.getPatternNumberOriginal().compareTo(currentPattern.getPatternNumberCurrent())!=0) {
				this.append(SetCommand.create(
					this.editingDomain,
					currentPattern,
	...
	private void syncPartsWithSamples(EList<? extends PartWithSample> partWithSampleList) {
	...
	private void syncPatternSetParameters() {
		...
		patternSetParameters = this.esxFile.getGlobalParameters().getPatternSetParameters();
		
		for(int i=0; i<patternSetParameters.size(); i++) {
			currentPatternPointer = patternSetParameters.get(i).getPatternPointer();
			try {
				currentPattern = esxFile.getPatternFromPointer(currentPatternPointer);
			} catch (Exception e) {
				currentPattern = null;
			}
			if(currentPattern!=null) {
				short newValue = (short) currentPattern.getPatternNumberCurrent().getValue();
